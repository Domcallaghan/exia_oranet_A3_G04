create or replace TYPE MEDOC IS object (nom_medoc VARCHAR2(38)
...
Create or replace  function F_Drug_list(    
    ...
    is
        ...
        n integer :=0;
    BEGIN
        for r in (select * from medicament where MED_REMBOURSABLE = refundable and CPH_ID=categmedic)
        loop
            l_emp_medoc.extend;
            n := n+1;
            l_emp_medoc(n) := MEDOC(r.MED_NOM,
                r.MED_NUM_LOT,
                r.CPH_ID,
